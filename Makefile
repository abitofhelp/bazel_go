...
expand_golang_build:
	$(BZLCMD) query $(CMD_WORKSPACE) --output=build
...
go_unit_test:
	$(BZLCMD) test --test_output=all --test_verbose_timeout_warnings "main_test"
...
sync_from_gomod: go_mod_download go_mod_tidy go_mod_vendor go_mod_verify gazelle_update_repos
